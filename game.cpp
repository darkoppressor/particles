...
void Game::set_camera(){
    ...
    if(camera.x+camera.w>(double)world.width*camera_zoom){
        camera.x=(double)world.width*camera_zoom-camera.w;
    }
    if(camera.y<0.0){
        camera.y=0.0;
    }
    if(camera.y+camera.h>(double)world.height*camera_zoom){
        camera.y=(double)world.height*camera_zoom-camera.h;
    }
