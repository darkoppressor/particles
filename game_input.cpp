    ...
    if(in_progress){
        ...
        if(event.type==SDL_MOUSEWHEEL){
            if(!event_consumed && event.wheel.y>0){
                if(++game.world.current_particle_type>engine_interface.particle_types.size()-1){
                    game.world.current_particle_type=0;
                }

                event_consumed=true;
            }
            else if(!event_consumed && event.wheel.y<0){
                if(game.world.current_particle_type==0){
                    game.world.current_particle_type=engine_interface.particle_types.size()-1;
                }
                else{
                    game.world.current_particle_type--;
                }
            ...
            if(!event_consumed && event.button.button==SDL_BUTTON_LEFT){
                uint32_t PARTICLE_SPAWN_RADIUS=5;

                uint32_t grid_x=(uint32_t)floor(((game.camera.x+(double)mouse_x)/game.camera_zoom)/(double)game.world.particle_size);
                uint32_t grid_y=(uint32_t)floor(((game.camera.y+(double)mouse_y)/game.camera_zoom)/(double)game.world.particle_size);

                game.world.spawn_particles(engine_interface.particle_types[game.world.current_particle_type].name,Collision_Rect(grid_x-PARTICLE_SPAWN_RADIUS,grid_y-PARTICLE_SPAWN_RADIUS,PARTICLE_SPAWN_RADIUS*2,PARTICLE_SPAWN_RADIUS*2));
            }
        }
    }
